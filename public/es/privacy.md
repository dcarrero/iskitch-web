# Política de privacidad — iSkitch

> Cómo trata iSkitch tus datos: no recopila ninguno. Tus capturas se quedan en tu Mac y la app nunca se conecta a nuestros servidores.

**Fecha de entrada en vigor:** 28 de mayo de 2026

Esta política explica cómo la aplicación **iSkitch** para macOS trata tus datos. La app está desarrollada y distribuida por **Color Vivo Internet, S.L.**, sociedad española con domicilio en Madrid (España).

## Resumen

- **No recopilamos datos personales.** iSkitch no recoge, almacena ni transmite datos personales a nosotros ni a terceros.
- **Sin analítica ni seguimiento.** La app no incluye telemetría, SDKs de analítica ni servicios de informes de fallos.
- **Tus capturas se quedan en tu Mac.** Las capturas, anotaciones y ediciones se procesan localmente y no salen de tu dispositivo salvo que tú decidas compartirlas.
- **Grabación de pantalla solo a petición.** macOS solicita este permiso únicamente cuando inicias una captura.

## Qué guarda la app en tu dispositivo

iSkitch guarda una pequeña cantidad de información en tu Mac para recordar tus preferencias entre sesiones:

- **Preferencias** en `UserDefaults`: formato de exportación por defecto, carpeta de guardado, color y estilo de flecha por defecto, apariencia (claro/oscuro/sistema), idioma y otras opciones de la interfaz.
- **Estado de la ventana**: tamaño y posición del editor para que la próxima vez se abra donde lo dejaste.
- **Capturas y archivos guardados**: solo cuando los exportas explícitamente, y solo en el destino que tú elijas.

Nada de esto se nos envía.

## Qué envía la app por internet

iSkitch **no realiza ninguna conexión saliente por sí misma**. No operamos servidores que reciban tus datos.

El permiso opcional `com.apple.security.network.client` se declara únicamente para que el compartir iniciado por ti a través de la hoja nativa de macOS funcione correctamente (por ejemplo, enviar una captura por Mail o Mensajes). Lo que ocurra después de elegir un destino lo gestiona esa extensión, no iSkitch.

## Permisos que iSkitch puede solicitar

- **Grabación de pantalla** (necesario): se concede en *Ajustes del Sistema ▸ Privacidad y seguridad ▸ Grabación de pantalla*. macOS usa este permiso para que iSkitch pueda leer los píxeles del área que selecciones con ⌥⌘4, de una ventana o de la pantalla completa. La imagen resultante se queda en tu Mac.
- **Acceso a archivos** mediante los paneles estándar de abrir y guardar: limitado al archivo o carpeta que selecciones en cada momento.
- **Portapapeles y arrastrar y soltar**: cuando pulsas "Copiar" o arrastras una imagen fuera de iSkitch, se escribe la imagen aplanada en el portapapeles o el destino que elijas.

## Apple como tercero

iSkitch se distribuye a través de la **Mac App Store**. Apple actúa como responsable del tratamiento independiente para descargas, valoraciones, pagos y los datos de diagnóstico que decidas compartir con los desarrolladores desde la App Store. Consulta la [Política de privacidad de Apple](https://www.apple.com/legal/privacy/) para entender cómo trata esa información.

## Menores

iSkitch tiene una clasificación de **4+** en la App Store y es apta para cualquier público. No recopilamos datos a sabiendas de ninguna persona, incluidos los menores de 13 años.

## Tus derechos bajo el RGPD

Como iSkitch no realiza tratamiento del lado del servidor y nosotros no recibimos ninguno de tus datos, no existen datos personales en nuestro poder a los que puedas acceder, rectificar, portar o suprimir. Mantienes el control total sobre todo lo almacenado en tu Mac y puedes eliminar la app y sus preferencias en cualquier momento.

## Cambios en esta política

Si en algún momento cambiamos la forma en la que iSkitch trata los datos, esta página se actualizará con una nueva fecha de entrada en vigor antes de que el cambio llegue a las personas usuarias.

## Contacto

**Color Vivo Internet, S.L.**
Madrid, España
Email: hello@iskitch.com
Web: https://iskitch.com
